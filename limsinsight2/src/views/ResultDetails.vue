...
export default {
    ...
    computed: {
        // function returns a filtered version of the resultdetails array
        filtered() {
            // filter each resultdetail in resultdetails
            let filtered = this.resultdetails.filter(resultdetail => {
                // returns true if for EVERY key in filter, there is a substring of that key's value
                // within the corresponding resultdetail's key  
                return Object.keys(this.filter).every(key => 
                    String(resultdetail[key]).includes(this.filter[key])
                );
            });
