...
func UpdateResult(result *models.Result) bool {
	stmt, err := database.DBUpdate.Prepare("UPDATE `JTree`.`results` SET `failed_regions` = ?,`mean_depth_of_coveage` = ?,`mlpa_pcr` = ?,`mutation` = ?,`overall_hotspots_threshold` = ?,`overall_quality_threshold` = ?,`results_id` = ?,`uid` = ?,`verification_pcr` = ?,`experiment_id` = ? WHERE `results_id` = ?;")
	if err != nil {
		log.Fatal(err)
	}
	outcome, err := stmt.Exec(
