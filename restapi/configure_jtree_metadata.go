...
func configureAPI(api *operations.JtreeMetadataAPI) http.Handler {
	...
	database.DBSelect = database.Init(c.Database.Host, c.Database.Selectuser+":"+c.Database.Selectpass+"@tcp(127.0.0.1:3306)/"+c.Database.Name+"?parseTime=true", database.DBSelect)
	database.DBUpdate = database.Init(c.Database.Host, c.Database.Updateuser+":"+c.Database.Updatepass+"@tcp(127.0.0.1:3306)/"+c.Database.Name+"?parseTime=true", database.DBUpdate)
