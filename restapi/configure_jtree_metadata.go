...
func configureAPI(api *operations.JtreeMetadataAPI) http.Handler {
	...
	database.DBSelect = database.Init(c.Database.Host, c.Database.Selectuser+":"+c.Database.Selectpass+"@tcp(172.18.0.3:3306)/"+c.Database.Name+"?parseTime=true", database.DBSelect)
	database.DBUpdate = database.Init(c.Database.Host, c.Database.Updateuser+":"+c.Database.Updatepass+"@tcp(172.18.0.3:3306)/"+c.Database.Name+"?parseTime=true", database.DBUpdate)
